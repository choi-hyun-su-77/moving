...
class Move_straight(Node):
    ...
    def aruco_sub(self, submsg):
        ...
        self.aruco_x = submsg.poses[0].position.x
        # self.angle = atan2(submsg.poses[0].position.y - self.y, submsg.poses[0].position.x - self.x)
        # self.get_logger().info(f'{submsg.poses[0].position.y}, {submsg.poses[0].position.x}')
        # self.get_logger().info(f'{self.angle}')
        # self.get_logger().info('***************** start_1 *****************')
        # self.get_logger().info(f'{submsg.poses[0].position.y}')
        # self.get_logger().info(f'{self.x}')
        # self.get_logger().info('****************** end_1 ******************')
    ...
    def update(self):
        msg = Twist()
        if self.moving:
            # self.get_logger().info('***************** start_2 *****************')
            # self.get_logger().info(f'{self.origin_y}')
            # self.get_logger().info('****************** end_2 ******************')
            #self.get_logger().info(f'{self.origin_x}, {self.origin_y}')
            # self.get_logger().info(f'elapse: {self.elapsed_dist()}')
            if self.aruco_x > 0:
                msg.angular.z = -self.aruco_x * 20
            elif self.aruco_x < 0: 
                msg.angular.z = -self.aruco_x * 20
